...
1. 起一个 3 Agent 的 GroupChat，先只用 `max_round` 兜底，跑一个开放性任务。
2. 观察它是"聊完了"还是"被轮次砍断了"。
3. 加三重终止：显式终止词、无新信息检测（连续两轮无新增结论）、轮次硬上限。

**验收标准**：正常任务由终止词收尾，异常任务由无新信息检测提前收尾，硬上限只在兜底时触发（且日志能区分是哪一种）。
