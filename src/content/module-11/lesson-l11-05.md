...
先回到多 Agent 拓扑对比面板，看清不同结构各自的协调成本——链式几乎零协调、网状最高。理解了拓扑的"固有开销"，再看失败模式就更清楚根因在哪：
...
**经验区间**（行业经验性观察，非严格实验结论）：多数生产多 Agent 系统的常见规模是 **2-4 个 Agent**。超��� 5 个，要非常清楚"这第 5、6 个 Agent 带来的具体收益是什么"，否则它只会增加协调成本和失败面。
...
量一次多 Agent 的边际收益，别凭"感觉更强"。

1. 同一批 15 个任务，分别用 1 / 2 / 4 个 Agent 的方案跑。
2. 记录三列：任务成功率、总 token、端到端耗时。
3. 算出每提升 1 个百分点的成功率，多花了多少钱。

**验收标准**：你的数据能回答"值不值"。如果 4 Agent 比单 Agent 贵 5 倍只涨 3 个点，正确的工程决定是退回单 Agent。
