...
> 反模式：**每轮对话都把图片重发一遍**。用户问完"图里有几人"再问"都拿什么设备"，第二轮不必重新编码和发送图片——将之前的 messages（含图片）一并传给 API 即可（API 本身无状态，但客户端的完整 messages 历史携带了上下文）。即便要重发，也该缓存而非重新编码。

### 动手 5 分钟

给图表数据提取加一道自动校验。

1. 让模型从一张柱状图提取 JSON 数据。
2. 加校验：数值是否在坐标轴范围内、类别数是否与图例一致、百分比是否加总接近 100。
3. 校验失败时带着具体错误重问一次。

**验收标准**：你能造出一个"模型输出格式合法但数值明显错误"的样例，并且你的校验能把它拦住——格式对不等于内容对。
