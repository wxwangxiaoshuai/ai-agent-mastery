...
语音链路比纯文本贵数倍：STT 按时长、TTS 按字符，再叠加 LLM。下面的三角优化器聚焦其中 **LLM 段**——模型档位 + Prompt Caching / 路由 / 流式等杠杆，看成本、延迟、质量如何互搏（STT/TTS 费率见下文清单，不在本交互里）：

::interactive{type="costLatency"}
...
1. 先实现串行版（录完 → STT 全文 → LLM 全文 → TTS），测从说完到听到第一个字的时间。
2. 改成流式重叠：STT 边听边出、LLM 首句就送 TTS。
3. 加打断：用户开口即停止当前播放并丢弃未播队列。

**验收标准**：首字延迟明显下降，且打断后旧回复不会在新回复之后"迟到"播出来。
