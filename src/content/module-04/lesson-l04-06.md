...
给 Naive RAG 加一层最便宜的高级范式：检索质量自检。

1. 在检索之后、生成之前插一步 LLM 判断：检索结果对回答这个问题是"够 / 不够 / 无关"。
2. "不够"时触发一次查询改写并重检索；"无关"时直接回复"知识库中没有相关信息"。
3. 用 5 个知识库里根本没有答案的问题测试。

**验收标准**：那 5 个问题全部走到拒答分支，没有一个被幻觉编出答案——这是 RAG 从"能用"到"敢用"的分界线。
