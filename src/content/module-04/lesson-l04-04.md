...
> **说明**：表中数字为量级示意，非特定数据集/模型实测。实际值因数据集、Embedding 模型、Reranker 选择而异。本节"动手 5 分钟"会引导你在自己的数据上实测这些数字。
>
> 召回率：top-k 中包含正确答案的比例。精确率：top-n 中正确答案的占比。混合检索提升召回（找得全），Reranking 提升精确（排得准）。

### 动手 5 分钟

量化混合检索和 Reranking 各自贡献了多少。

1. 用同一套 20 个问题，跑四个配置：纯向量、纯 BM25、混合、混合+Rerank。
2. 记录每个配置的 top-3 命中率和端到端延迟。
3. 调一次混合权重（0.3/0.7 → 0.7/0.3），看命中率往哪边动。

**验收标准**：你能说出在你的数据上，Rerank 带来的命中率增益值不值那几十到几百毫秒的延迟。
