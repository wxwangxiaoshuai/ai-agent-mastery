...
先把整条流水线走一遍，感受一下 RAG 从来不是"塞进向量库然后搜"这么简单：
...
用 30 行代码搭一个 Naive RAG，然后亲手看它失败。

1. 取 5 篇你熟悉的文档，固定 500 字符切块，塞进内存里的向量列表（先不用向量库）。
2. 实现 `retrieve(q, k=3)` + 拼进 Prompt，跑通 3 个问题。
3. 现在提 3 个"注定失败"的问题：跨文档对比类、需要精确编号类、答案不在文档里的。

**验收标准**：你能对着输出指出每个失败属于本节哪一种失败模式（切分不当 / 检索不准 / 无法拒答），而不是笼统地说"效果不好"。
