...
先用向量相似度搜索面板建立直觉，再往下看它背后的数学：

::interactive{type="embedding"}
...
亲手撞一次 Embedding 的语义盲区。

1. 用同一个 Embedding 模型，算这三对的余弦相似度：（"涨价"，"提价"）、（"涨价"，"降价"）、（"ISO 27001"，"ISO 9001"）。
2. 观察第二、三对的分数——反义词和形近编号的相似度往往高得离谱。
3. 加一层元数据过滤或关键词条件，让第三对不再互相干扰。

**验收标准**：你能说出"为什么纯向量检索搜不准型号、编号、否定语义"，这正是下一节混合检索存在的理由。
