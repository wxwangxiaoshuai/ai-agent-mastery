...
先看三种策略切同一段文字的结果，红色标记是被硬切断的句子。交互里的第三种是**父子双索引**（父块保上下文、子块保精度）；正文「策略 3」讲的是 **RecursiveCharacterTextSplitter 式递归分隔符**——两者都叫过「层级」，但算法不同，别混为一谈：

::interactive{type="chunking"}

### 为什么分块很重要

```
文档（10000 字）
  ↓ 分块策略 A（500 字/块）→ 20 个 chunk → 检索精度 75%（量级示意，非实测）
  ↓ 分块策略 B（200 字/块）→ 50 个 chunk → 检索精度 82%（量级示意，非实测）
  ↓ 分块策略 C（语义分块）  → 35 个 chunk → 检索精度 88%（量级示意，非实测）
```

> **说明**：上述精度数字为量级示意，非特定数据集实测结果。实际精度因文档类型、Embedding 模型、分块实现而异。

同一个文档、同一个 Embedding 模型，仅仅改变分块策略，检索精度就可能差出 10 个以上百分点。
...
用同一份文档跑三种分块，用检索命中率而不是肉眼来评判。

1. 取一份含标题层级的长文档（技术手册最好），分别用固定 512、语义分块、递归分块切一遍。
2. 准备 10 个问题，人工标注每题的正确答案落在原文哪一段。
3. 三种切法各跑一次检索，统计 top-3 里包含正确段落的比例。

**验收标准**：你的表格能回答"我的文档该用哪种切法"，并且你能解释为什么被切断的句子数和命中率不完全正相关。
