...
具体型号几个月就换一代，但"这类任务该花多少钱"这件事很稳定。所以本课程统一用**档位**
来讨论选型：极小档做路由分类、小档做抽取摘要、中档做生产主力、大档留给真正复杂的推理。
写代码时才落到具体标识——而全站出现过的每一个标识都登记在 `src/data/models.ts` 里，
由校验脚本的 C5 守着，正文里写错一个型号 CI 就会失败。

::interactive{type="modelTiers"}
...
为你手上一个真实场景做一次选型决策，并把它写下来。

1. 用一句话描述任务（例："把用户工单分类到 8 个类别之一"）。
