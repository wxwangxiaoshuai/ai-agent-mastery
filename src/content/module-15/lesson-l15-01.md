...
Agent 生产部署架构的静态全景——网关→鉴权限流→队列→Agent 服务→语义缓存/熔断→模型 API→向量库→监控：

::interactive{type="deploymentArch"}
...
给你的 Agent 加一层语义缓存，并量它的命中率。

1. 实现：请求先做 embedding，与缓存中的历史问题算相似度，超过阈值直接返回缓存答案。
2. 用 100 条真实（或模拟）请求跑一遍，记录命中率与省下的钱。
3. 把阈值从 0.95 降到 0.85，看命中率和错误命中（返回了不该返回的答案）怎么变。

**验收标准**：你能给出你场景的阈值取值和依据，并造出一个"相似但答案完全不同"的问题对来说明阈值定太低的风险。
