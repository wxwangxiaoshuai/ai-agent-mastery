...
把一个"转圈等 30 秒"的 Agent 改成可视的流式体验。

1. 后端用 SSE 推三类事件：`step`（我在做什么）、`token`（正文增量）、`done`。
2. 前端渲染步骤时间轴 + 正文流式追加。
3. 加一个"停止"按钮，点击后后端真的中止（不只是前端不显示）。

**验收标准**：点停止后服务端日志显示任务确实终止、token 停止计费——前端假装停止是最常见的假流式。
