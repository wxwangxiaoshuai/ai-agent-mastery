...
同一个任务用 CrewAI 和 LangGraph 各写一遍，比较可读性而不是性能。

1. 选一个天然有角色分工的任务（调研 → 撰写 → 校对）。
2. 两个框架各实现一版，统计代码行数和你花的时间。
3. 现在加一个需求：校对不通过时退回撰写，最多退两次。

**验收标准**：你能说出加上这个回退需求后，哪个框架改起来更顺——角色模型擅长表达分工，图模型擅长表达控制流。
