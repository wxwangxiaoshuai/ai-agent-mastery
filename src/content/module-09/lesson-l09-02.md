...
把每一个 Docker 安全旋钮都拧一次，验证它到底防住了什么。

1. 起一个基线容器，依次开关：`network_disabled`、`mem_limit`、`pids_limit`、`read_only`、`cap_drop=ALL`、非 root 用户。
2. 每个旋钮配一段攻击代码（外连、内存炸弹、fork 炸弹、写文件、提权）。
3. 记录旋钮关闭时发生什么、打开时怎么被拒。

**验收标准**：你有一张"旋钮 → 它挡住的攻击"对照表，而不是一份抄来的配置。
