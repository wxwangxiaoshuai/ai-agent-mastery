...
Computer Use 的前置是多模态感知。先用对比面板感受图/音/视频相对纯文本多带走什么信息（以及文本-only 会丢什么）；正文再讲截图 → 决策 → 鼠标/键盘的 CU 循环：
...
在一次性虚拟机里跑一个最小的 Computer Use 任务，并记录失败点。

1. 任务定为可验证的小事（如在记事本里写一句话并保存到指定路径）。
2. 跑 10 次，统计成功率，并记录每次失败卡在哪一步。
3. 加两个增强：每步操作后截图校验、关键步骤前设检查点。

**验收标准**：你能说出 GUI Agent 的成功率与 API 调用的成功率相差多少个数量级——这决定了它只适合什么场景。**务必在隔离环境操作，不要授权真实账号和支付相关界面。**
