...
为你的 Agent 写一张 Agent Card，用发布者的视角审视它。

1. 按 A2A 的字段写：能力描述、输入输出 schema、认证方式、限流、SLA。
2. 交给一个不了解你系统的人（或另一个 Agent），只看这张卡能不能正确调用。
3. 补上最容易漏的三项：失败时返回什么、超时多久、调用方需要付费吗。

**验收标准**：对方只凭这张卡就能调通一次，且能预判失败时会拿到什么——这就是"可互联"的最低门槛。
