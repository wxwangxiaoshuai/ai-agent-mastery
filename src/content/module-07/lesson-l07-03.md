...
在 Agent 执行到一半时把它杀掉，看它能不能活过来。

1. 给你的 Agent 加上每步结束后写 checkpoint。
2. 跑一个 8 步任务，在第 5 步时 `Ctrl+C`。
3. 重新启动，从 checkpoint 恢复。

**验收标准**：恢复后不重复执行已完成的副作用操作（发邮件、写库这类不能重来的动作），且最终结果与不中断时一致。
