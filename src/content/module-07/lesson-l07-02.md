...
1. 写一个失败率 100% 的假 API，用固定间隔重试 5 次、20 个并发同时打。
2. 打印每次重试的时间戳，观察请求在时间轴上是怎么"撞成一堆"的。
3. 换成指数退避 + Jitter，再看一次时间轴。

**验收标准**：加 Jitter 后请求在时间上明显分散，并且你能解释为什么没有 Jitter 的指数退避依然会同步共振。
