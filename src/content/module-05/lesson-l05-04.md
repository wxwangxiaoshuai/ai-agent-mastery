...
1. 选一个需要 6 步以上的任务（如"调研三个竞品并出对比表"）。
2. 分别用 ReAct 和 Plan-and-Execute 实现，各跑 3 次。
3. 记录：LLM 调用次数、总 token、端到端耗时、任务成功率。

**验收标准**：你能说清 Plan-and-Execute 省下的调用次数，是以什么为代价换来的（提示：计划过时了怎么办）。
