...
给你手写的 Agent Loop 装上三个安全阀，然后逐个触发它们。

1. 加 `max_steps=8`、`max_tokens_total`、`max_wall_seconds` 三个上限，任一触发即中止并返回已有结果。
2. 造一个会让 Agent 死循环的任务（例如工具永远返回"信息不足"）。
3. 观察三个阀门哪个先响。

**验收标准**：Agent 在异常任务下能停下并给出可读的中止原因，而不是无限烧钱——这一条比让它答对更重要。
