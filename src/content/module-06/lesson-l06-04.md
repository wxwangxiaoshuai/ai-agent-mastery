...
接一个现成的 MCP Server，感受"即插即用"到底有多即插即用。

1. 在 Claude Desktop 或任一 MCP Client 里接入官方 filesystem server，限定一个只读目录。
2. 提三个请求：读目录内文件、读目录外文件、写文件。
3. 观察后两个被拒绝的方式和提示信息。

**验收标准**：你能说清这次的权限边界是由谁强制的（Server 还是 Client），以及如果 Server 自己不做校验会发生什么。
