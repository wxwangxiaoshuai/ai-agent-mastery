...
测一次压缩的真实代价——压缩从来不是免费的。

1. 取一段 8000 token 的长对话历史，分别用摘要压缩和选择性压缩压到 2000 token。
2. 准备 10 个只有靠历史细节才能答对的问题（如"我第三次提到的预算是多少"）。
3. 用原始历史、摘要版、选择性版各答一遍，统计答对数。

**验收标准**：你有一张三行表格，能直观说明"省了多少 token"换来"丢了多少信息"，并据此决定你的场景该用哪种。

### 要点总结

- 上下文压缩的目标：用更少 token 传达同等信息量，同时缓解"Lost in the Middle"效应
- 三种方法各有适用场景：摘要压缩（通用但有失真）、选择性压缩（快速但粗粒度）、LLMLingua（精细但需额外模型）
- 摘要压缩用 LLM 改写，适合对精度要求不高的场景；选择性压缩按相关性分数截断，适合有明确 query 的检索场景
- 生产环境推荐混合管道：选择性压缩 → LLMLingua → 摘要压缩，逐级降级，每一级有独立的触发阈值
- 对话历史用特殊策略：近期完整保留 + 旧历史摘要替代，平衡"上下文连贯性"和"token 成本"
- 压缩是"信息保真度 vs token 成本"的权衡——没有免费午餐，压缩比越高失真风险越大
- LLMLingua 的核心思路是用小模型估算 token 级信息量，删掉信息密度最低的 token，最大压缩比可达 20x
