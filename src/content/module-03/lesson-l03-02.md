...
先通过下面的流程图感受一下 Context 组装的全过程——从各个数据源到最终发送给 LLM 的完整消息数组：

::interactive{type="contextAssembly"}
...
1. 挑一次你的 Agent 真实调用，把送进模型的完整内容 dump 到文件。
2. 按五大组成部分（系统指令 / 工具定义 / 检索内容 / 对话历史 / 当前输入）逐段标注，统计每部分的 token 占比。
3. 找出占比最大的那一块，问自己：它有多少是每次都必需的？

**验收标准**：你能指出至少一处"每次都传但大部分时候用不上"的内容，并说明把它改成按需注入的具体做法。
