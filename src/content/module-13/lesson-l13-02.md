...
1. 把 L13-01 的评测集提交进仓库，给数据集打版本号。
2. 写一个跑分脚本，低于基线分数 5% 时 exit 1。
3. 提一个会退化的 PR（删掉 Prompt 里一条约束），确认 CI 拦住了。

**验收标准**：CI 失败信息里能直接看出是哪几条用例、退化了多少分，而不是只有一个"测试失败"。
