...
1. 给你的 LLM 客户端加一层录制包装，把真实请求与响应存成 fixture。
2. 回放模式下按请求哈希命中 fixture，未命中时明确报错（而不是偷偷走真实调用）。
3. 用 fixture 跑一遍完整 Agent 轨迹测试。

**验收标准**：断网状态下测试全绿，且当你改动了 Prompt 导致请求哈希变化时，测试会明确提示"需要重新录制"而不是静默通过。
