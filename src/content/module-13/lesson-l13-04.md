...
先用下面的测试台开关各层护栏、换不同攻击样本，理解"没有单层护栏能挡住所有攻击"：
...
量一次护栏的误杀率——这是护栏最常被忽略的代价。

1. 准备 20 条正常输入 + 10 条恶意输入。
2. 开启全部护栏跑一遍，统计四个数：拦住的恶意、漏过的恶意、误杀的正常、正常通过。
3. 逐条放宽护栏，看误杀率和漏过率怎么此消彼长。

**验收标准**：你能给出你场景的取舍立场——是宁可误杀还是宁可漏过——并说明这个立场由什么业务后果决定。
