...
用一条 trace 找出你 Agent 里最贵和最慢的那一步。

1. 给 Agent 的每一步埋 span，记录耗时、输入输出 token、模型名。
2. 跑 20 次真实请求，聚合出每一步的 P50/P95 耗时和平均成本。
3. 排序，圈出耗时最长的一步和花钱最多的一步——它们通常不是同一步。

**验收标准**：你能说出优化哪一步性价比最高，并给出数字支撑（如"检索占 P95 的 60% 但只占成本的 5%"）。
