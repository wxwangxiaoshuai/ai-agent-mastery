...
Agent 测试也要量化——覆盖率与分层门禁。用 CI 配置把阈值写死，不同严格程度直接体现在「不达标就阻断」：
...
按测试金字塔给你的 Agent 补齐三层测试。

1. 单元层：测确定性部分（Prompt 渲染、工具参数校验、结果解析），全部 mock LLM。
2. 集成层：测 Agent 与工具的协作，用假工具但真循环。
3. 端到端：3 条最关键的用户路径，允许慢、允许贵，但必须有。

**验收标准**：单元层秒级跑完且零 API 消耗，端到端层不超过 3 条——金字塔倒过来的测试套件没人愿意跑。
