...
const layers: DiagramLayer[] = [
  { id: 'edge', label: '边缘层 —— 网关 + 限流 + 鉴权', y: 8, height: 10, color: 'fuchsia' },
  { id: 'queue', label: '队列层 —— 削峰填谷', y: 22, height: 10, color: 'amber' },
  { id: 'agent', label: 'Agent 服务层 —— 推理 + 工具调用', y: 36, height: 10, color: 'brand' },
  { id: 'model', label: '模型层 —— LLM API + 缓存', y: 50, height: 10, color: 'emerald' },
  { id: 'data', label: '数据层 —— 向量库 + 日志 + 监控', y: 64, height: 10, color: 'ink' },
]

const nodes: DiagramNode[] = [
  { id: 'gw', label: 'API\n网关', x: 5, y: 10, color: 'fuchsia' },
  { id: 'auth', label: '鉴权\n限流', x: 25, y: 10, color: 'fuchsia' },
  { id: 'mq', label: '消息\n队列', x: 25, y: 24, color: 'amber' },
  { id: 'worker', label: 'Agent\nWorker', x: 5, y: 38, color: 'brand' },
  { id: 'tool', label: '工具\n执行器', x: 25, y: 38, color: 'brand' },
  { id: 'cache', label: '语义\n缓存', x: 45, y: 52, color: 'emerald' },
  { id: 'llm', label: 'LLM\nAPI', x: 65, y: 52, color: 'emerald' },
  { id: 'vec', label: '向量\n数据库', x: 5, y: 66, color: 'ink' },
  { id: 'log', label: '日志\n追踪', x: 25, y: 66, color: 'ink' },
  { id: 'monitor', label: '监控\n告警', x: 45, y: 66, color: 'ink' },
  { id: 'cb', label: '熔断器', x: 85, y: 38, color: 'danger' },
]
